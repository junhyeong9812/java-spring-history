...
## 시대적 배경
Java 19는 **세 거대 프로젝트(Loom, Panama, Amber)의 결실이 동시에 가시화된** 릴리스다. 특히 2017년 말 시작되어 약 5년간 진행되어 온 Project Loom의 가상 스레드가 처음으로 프리뷰로 공개되면서, "스레드 하나에 OS 스레드 하나"라는 Java 동시성의 근본 제약을 깨뜨릴 길이 열렸다.
