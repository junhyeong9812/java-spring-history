...
```java
// JDK 18부터: 명시적으로 charset을 지정하지 않아도 UTF-8로 동작
import java.nio.file.*;

var path = Path.of("note.txt");
Files.writeString(path, "안녕하세요 🚀"); // UTF-8로 기록
String text = Files.readString(path);      // UTF-8로 읽기
...
- 기본은 현재 디렉토리를 `127.0.0.1:8000`에 GET 전용·읽기 전용으로 서빙한다. CGI나 동적 콘텐츠는 지원하지 않는다.
