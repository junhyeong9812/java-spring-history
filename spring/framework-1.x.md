...
- 주요 마이너 버전과 시기: 0.9(2003, 책과 함께 공개) → 1.0(2004-03) → 1.1(2004) → 1.2(2005)
- 최소 자바 버전(baseline): J2SE 1.3 / 1.4 (Java 5 이전, 어노테이션·제네릭 미사용)
...
## 설정 스타일의 변화
이 시대는 **100% XML 구성**이다. 어노테이션도 제네릭도 없었다(Java 1.4 기준). 모든 빈, 의존성, AOP, 트랜잭션이 XML에 명시적으로 선언되었다. DTD 기반 검증을 사용했으며, 이 장황함이 훗날 2.x의 XML 네임스페이스 간소화와 3.x의 Java Config 등장 동기가 된다.

## 마이너 버전별 변화
- 1.0 (2004-03): 첫 정식 릴리스. IoC 컨테이너, AOP, JDBC/트랜잭션 추상화, Web MVC, ORM 통합 등 핵심 모듈 확립.
- 1.1 (2004): 컨테이너·AOP 안정화 및 성능 개선.
- 1.2 (2005): JDK 1.5(Java 5) 런타임에서의 동작 지원 강화, 소스 레벨 메타데이터(Commons Attributes) 기반 트랜잭션, AOP 개선. 이후 어노테이션 시대로 가는 징검다리.
